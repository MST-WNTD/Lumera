...
async function loadNotifications() {
    try {
        const response = await fetch('/organizer/notifications');
...
async function updateNotificationCount() {
    try {
        const response = await fetch('/organizer/notifications/unread-count');
        const data = await response.json();
...
async function deleteNotification(notificationId) {
    try {
        const response = await fetch(`/organizer/notifications/delete/${notificationId}`, {
            method: 'DELETE'
        });
...
async function handleNotificationClick(notificationId, redirectUrl) {
    try {
        // Mark as read
        await fetch(`/organizer/notifications/mark-read/${notificationId}`, {
            method: 'POST'
        });

        // Validate the redirect URL before navigating
        if (redirectUrl) {
            // Check if it's a booking details URL
            const bookingMatch = redirectUrl.match(/\/organizer\/bookings\/details\/(\d+)/);

            if (bookingMatch) {
                const bookingId = bookingMatch[1];

                // Verify the booking exists before redirecting
                const verifyResponse = await fetch(`/organizer/api/verify-booking/${bookingId}`);
            ...
            // No redirect URL, just refresh notifications
            updateNotificationCount();
            loadNotifications();
        }
    } catch (error) {
...
async function markAllAsRead() {
    try {
        const response = await fetch('/organizer/notifications/mark-all-read', {
            method: 'POST'
        });
